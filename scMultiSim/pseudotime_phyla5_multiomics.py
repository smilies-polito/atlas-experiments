...
if __name__=="__main__":
	...
	n_macrostates = 5
	albero = "phyla5"
	early_cell = "cell1" #phyla5
	#eary_cell = "cell640" #phyla3
	
	data_path =  os.path.join(os.getcwd(), f"{albero}")
	tsv_path = os.path.join(os.getcwd(), f"pseudotime_{albero}_multiomics.csv")
	failures = []

	results = {"albero":albero, "n_cellule":1000, "GRN_type":"GRN_100", "sigma_cif":None, "diff_cif_fraction":None, "modello":"multiomics", "fixed_terminal":False, "knn_atac":None, "knn_rna":None, "wnn":None, "algoritmo":"cellrank_pseudotime_kernel", "n_waypoints":None, "knn_waypoints":None, "n_macrostates":n_macrostates, "velocity_algorithm":None, "pruning_type":threshold_scheme, "pearson_kl_statistics":None, "pearson_kl_pvalue":None, "kendall_pseudotime_statistics":None, "kendall_pseudotime_pvalue":None, "pearson_entropy_statistics":None, "pearson_entropy_pvalue":None, "f1_cosine":None, "f1_euclidean":None}

	# read selected cells pseudotime kernel 
	cell_path = os.path.join(data_path, f"selected_cells_{albero}.json")
	...
	#			g.plot_macrostate_composition(key="rna:pop", show=False, save=os.path.join(saving_folder_noterminal, f"macrostate_composition_{n_macrostates}.png"), title=f"Macrostate Composition {n_macrostates}")
	#			g.plot_coarse_T(annotate=True, save=os.path.join(saving_folder_noterminal, f"coarseT_{n_macrostates}.png"), title =f"Macrostates {n_macrostates}")
				g.compute_fate_probabilities(tol=1e-10, use_petsc=True, preconditioner="ilu", backend="threading", n_jobs=1)
	...
	failure_path = os.path.join(saving_folder, "failures.tsv") 
