...
if __name__ == "__main__":
	...
	for values in product(*grid.values()):
		...
		# MULTIOMICS RUN
			...
			g.plot_fate_probabilities(same_plot=True, save=os.path.join(save_multiomics, f"fateProb_set.png"), title="Fate Probabilities", show=False)
			...
			g.set_terminal_states(cells["terminal"])
			g.compute_fate_probabilities(tol=1e-10, use_petsc=True, preconditioner="ilu")
			g.plot_fate_probabilities(same_plot=True, save=os.path.join(save_rna, f"fateProb_set.png"), title="Fate Probabilities", show=False)
