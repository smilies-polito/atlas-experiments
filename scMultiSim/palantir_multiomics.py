	...
	for diff_cif_fraction, sigma_cif, knn_rna, knn_atac, wnn in product(*grid_preprocessing.values()):
		...
		for percentage, knn_waypoints in product(*grid_palantir.values()):
			...
			try:
				...
				if fix_terminal:	
					pw.run_palantir(data, early_cell = early_cell, num_waypoints=n_waypoints, terminal_states = terminal_states, knn=knn_waypoints, seed=seed)
				else:
					pw.run_palantir(data, early_cell = early_cell, num_waypoints = n_waypoints, knn=knn_waypoints, seed=seed)	
...
#				plot_palantir_results(data = data, modality_key = None, embedding_key = "X_umap", pseudo_time_key = "palantir_pseudotime", entropy_key = "palantir_entropy", fate_prob_key = "palantir_fate_probabilities", save= True, saving_path = saving_path)
