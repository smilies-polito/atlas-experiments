...
if __name__ == "__main__":
	...
	results["si_terminal"] = {}
	statistics, pvalue = compute_correlation(df, "pearson","pseudotime", "entropy")
	results["si_terminal"]["pearson_entropy"] = {"statistics":statistics, "pvalue":pvalue}
	statistics, pvalue = compute_correlation(df, "kendall_tau", "pseudotime", "entropy")	
	results["si_terminal"]["kendall_tau_entropy"] = {"statistics":statistics, "pvalue":pvalue}
	statistics, pvalue = compute_correlation(df, "pearson", "pseudotime", "KL")
	results["si_terminal"]["pearson_KL"] = {"statistics":statistics, "pvalue":pvalue} 
	statistics, pvalue = compute_correlation(df, "kendall_tau", "pseudotime", "KL")
	results["si_terminal"]["kendall-tau_KL"] = {"statistics":statistics, "pvalue":pvalue} 

	# compute f1 score
	results["f1"] = {}
	for terminal in cells["terminal"].keys():
		cosine, euclidean = compute_f1(df[termimal], ground_truth[terminal]
		results["f1"][terminal] = {"cosine":cosine, "euclidean":euclidean} 
	cosine, euclidean = compute_f1(dataframe[list(cells["terminal"].values())], ground_truth[list(cells["terminal"].keys())], aggregate=True)
	results["f1"]["total"] = {"cosine":cosine, "euclidean":euclidean}	
	...
	g = cellrank.estimators.GPCCA(kernel.kernel)
	g.compute_schur()
	g.compute_macrostates(n_states=n_states, cluster_key="rna:pop")
	g.predict_initial_states()
	g.predict_terminal_states(allow_overlap=True)
	g.plot_macrostate_composition(key="rna:pop", show=False, save = os.path.join(saving_folder, f"macrostate_composition_{n_states}.png"), title= f"Macrostate Composition {n_states}")
	...
	results["no_terminal"] = {}
	statistics, pvalue = compute_correlation(df, "pearson","pseudotime", "entropy")
	results["no_terminal"]["pearson_entropy"] = {"statistics":statistics, "pvalue":pvalue}
	statistics, pvalue = compute_correlation(df, "kendall_tau", "pseudotime", "entropy")	
	results["no_terminal"]["kendall_tau_entropy"] = {"statistics":statistics, "pvalue":pvalue}
	statistics, pvalue = compute_correlation(df, "pearson", "pseudotime", "KL")
	results["no_terminal"]["pearson_KL"] = {"statistics":statistics, "pvalue":pvalue} 
	statistics, pvalue = compute_correlation(df, "kendall_tau", "pseudotime", "KL")
	results["no_terminal"]["kendall-tau_KL"] = {"statistics":statistics, "pvalue":pvalue} 
