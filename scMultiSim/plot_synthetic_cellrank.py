...
if __name__=="__main__":
	...
	not_found = [(0.5, 0.1), (0.7,0.1), (0.9,0.1), (0.9,0.3), (0.9,0.5)]

	for model in model_list: 
		for values in itertools.product(*grid.values()):
			diff_cif_fraction, cif_sigma = values
			if (diff_cif_fraction, cif_sigma) in not_found:
				continue
	...
	for model in model_list:
		...
		values = grouped_df.apply(get_group_correlation, "pearson", "pseudotime", "entropy", return_pvalue=False).unstack(level="cif_sigma")
		pvalues = grouped_df.apply(get_group_correlation, "pearson", "pseudotime", "entropy").unstack(level="cif_sigma")
		...
		values = grouped_df.apply(get_group_correlation, "pearson", "pseudotime", "KL", return_pvalue=False).unstack(level="cif_sigma")
		pvalues = grouped_df.apply(get_group_correlation, "pearson", "pseudotime", "KL").unstack(level="cif_sigma")
