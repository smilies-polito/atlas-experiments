...
if __name__=="__main__":
	...
	albero = ...
	palantir_early_cell = ...
	results = {"albero":albero, "n_cellule":1000, "GRN_type":"GRN_100", "sigma_cif":None, "diff_cif_fraction":None, "modello":"multiomics", "fixed_terminal":fix_terminal, "knn_atac":None, "knn_rna":None, "wnn":None, "algoritmo":"Cellrank Pseudotime Kernel", "n_waypoints":None, "knn_waypoints":None, "n_macrostates":n_macrostates, "velocity_algorithm":None, "pruning_type":threshold_scheme, "pearson_kl_statistics":None, "pearson_kl_pvalue":None, "pearson_entropy_statistics":None, "pearson_entropy_pvalue":None, "f1_cosine":None, "f1_euclidean":None, "cpu_time": None, "wall_time":None, "cpu_time_kernel":None, "wall_time_kernel":None}
	
	#path declaration + additional files
    working_dir = "/home/scvemo"
	failures_path = os.path.join(working_dir, "results", f"failing_pseudoKernel_simulations.csv")
    tsv_path = os.path.join(working_dir, "results", f"pseudoKernel_simulations.csv")
	data_path = os.path.join(working_dor, "data", f"{albero}")

#	saving_folder = os.path.join(os.getcwd(), f"pseudotime_{albero}_multiomics", f"{diff_cif_fraction}_{sigma_cif}_{knn_rna}{knn_atac}{wnn}_rna")
...
#		os.mkdir(saving_folder)
	...
	for diff_cif_fraction, sigma_cif, knn_rna, knn_atac, wnn in product(*grid_preprocessing.values()):
		...
		for percentage, knn_waypoints in product(*grid_palantir.values()):
			...
			try:
				pw.run_palantir(data, early_cell = palantir_early_cell, num_waypoints = n_waypoints, knn=knn_waypoints, seed=seed)	
				data.update()
				...
				if fix_terminal:
					...
					g.compute_macrostates(n_states = n_macrostates, cluster_key="rna:pop")
...
#		 		plot images
#				saving_path= os.path.join(saving_folder, f"{n_waypoints}_{knn_waypoints}_{fix_terminal}")
#				if not os.path.exists(saving_path):
#					os.mkdir(saving_path)
#               g.plot_fate_probabilities(same_plot=True, save=os.path.join(saving_path, f"fateProbs.png"), title="Fate Probabilities", show=False)
#               if not fix_terminal:
#                   g.plot_macrostate_composition(key="pop", show=False, save=os.path.join(saving_folder, f"macrostates_composition_{n_macrostates}.png"), title=f"Macrostate Composition {n_macrostates}")
#                   g.plot_coarse_T(annotate=True, save = os.path.join(saving_folder, f"corseT_{n_macrostates}.png", title=f"Macrostates {n_macrostates}")
