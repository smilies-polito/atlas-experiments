...
def _hard_ai(f:pd.DataFrame, D:pd.DataFrame) -> pd.Series:
	...
	Parameters: 
	- f: pandas.DataFrame;
	- D: pandas.DataFrame; distances between cells
...
def terminal_state_silhouette(fates: pd.DataFrame, 
	...
	- soft_assignemnt: bool; Indicates wether to deal with pluripotent cells according to the soft assignment strategy (see later) or pseudotime_weight. Default True. 
	...
	if not soft_assignment and pseudotime is None:
			raise ValueError(f"Pseudotime weight strategy requires pseudotime for all cells to be provided")

	D = pd.DataFrame(cdist(fates.values, fates.values, metric = "euclidean"),
				index = fates.index,
				columns = fates.index)	
