...
import numpy as np
import pandas as pd
from core.utils import compute_branch_correlation, plot_branch_correlation

if __name__=="__main__":
	pseudotime = np.random.uniform(0,1,120)
	entropy = 1 + pseudotime/2
	groups = np.random.choice(["A", "B", "C"], 120)
	dataframe = pd.DataFrame({"entropy":entropy, "pseudotime":pseudotime, "groups":groups})
	...
	print(entropy.max())
	plot_branch_correlation(dataframe=dataframe, key1= "pseudotime", key2= "entropy", group_key = "groups", branch=branch, save=save, saving_path = saving_folder, title= "Pseudotime-Entropy scatter", xlabel = "pseudotime", ylabel="entropy", xlim = (0, 1.1), ylim = (0, dataframe["entropy"].max() + 0.01))
