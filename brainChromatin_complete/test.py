...
if __name__=="__main__":
	...
	data = mu.read_h5mu("/Users/lrcq/Documents/devtraj/preprocessing/brainChromatinGreenLeaf/data.h5mu")	
	...
	for ns in range(2,5):
		try:	
			...
			g.plot_macrostate_composition(key = cell_type_key, show=False, title=f"{ns} macrostates",
					save =f"composition_{ns}.png")
			g.plot_macrostates(which="initial", legend_loc="right", s=100, show=False,
					title = f"Initial, {ns} macrostates",
					save = f"initialstates_{ns}.png")
			g.plot_macrostates(which="terminal", legend_loc="right", s=100, show=False,
					title = f"Terminal, {ns} macrostates", 
					save = f"terminalstates_{ns}.png")
			g.compute_fate_probabilities(tol=1e-10, use_petsc=True, preconditioner = "ilu")
			g.plot_fate_probabilities(same_plot= True, title=f"{ns} macrostates", save=f"fate_{ns}.png", show=False)
			
			fate_path = os.path.join(os.getcwd(), f"fates_{ns}.tsv")
