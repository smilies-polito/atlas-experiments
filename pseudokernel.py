...
class PseudotimeKernelMuon():
	def __init__(self, data: mu.MuData, modality_key:Optional[str] = None, embedding_key : str = "X_umap", connectivity_key: str = "connectivities", pseudotime_key: str="pseudotime", group_key: Optional[Union[str, list]]=None):

		if modality_key is not None:
			if modality_key not in data.mod.keys():
				raise KeyError(f"{modality_key} not available")
			print("Considering {modality_key} modality")
