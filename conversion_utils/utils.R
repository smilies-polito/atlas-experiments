...
SaveSeuratCSV <- function(object, 
  ...
  if(verbose){print("Save topFeature")}
  top.feature <- as.data.frame(object[[assay]]@var.features)
  fwrite(x = top.feature, row.names = F,  col.names=F, file = file.path(path,"topfeature.csv"))
